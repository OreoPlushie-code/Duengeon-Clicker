...
func check_score():
	...
	elif seconds > 0 and score >= 50:
		print("You Win")
		$Timer.stop()
		$Label2.text = "You Win"
		$AnimationPlayer.play("wait time")
		await $AnimationPlayer.animation_finished
		$transition.change_scene("res://level_2_re.tscn")
	else:
		pass
...
func _on_power_pressed():
	if $power.text == """Use PowerUp""":
		score_increment = 2
		$"power timer".start()
		$power.text = "POWER UP IN USE"
		

func _on_power_timer_timeout():
	score_increment = 1
	$power.text = "PowerUp Not Available"
	$"power timer".stop()
